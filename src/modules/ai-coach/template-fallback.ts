...
export function templateAnswer(question: string, content: Record<string, unknown>): string {
  ...
  if (q.includes("stall") || q.includes("plateau") || q.includes("not progressing")) {
    if (s.plateaus?.length) {
      ...
      const sleepNote = s.sleep?.status === "low" ? ` Your sleep averages ${s.sleep.avg_hours}h, below the 7h needed for recovery.` : "";
      ...
      return `${p.name} has been stalled for ${p.weeks_stalled} weeks.${sleepNote}${volumeNote} Try changing rep range (e.g. 5x5 to 4x8) or adding 2-3 sets per week.`;
    }
  ...
  if (q.includes("focus") || q.includes("this week") || q.includes("should i")) {
    const tips: string[] = [];
    if (s.plateaus?.length) tips.push(`Address ${s.plateaus[0].name} stall, try a rep range change.`);
  ...
  if (q.includes("consistent") || q.includes("streak") || q.includes("workout")) {
    ...
    const consistencyNote = s.consistency.status === "good" ? "on pace with your target" : "below your weekly target";
    return `You've done ${s.consistency.workouts_this_week} workouts this week (${s.consistency.avg_per_week_4w}/wk avg over 4 weeks). Current streak: ${s.consistency.streak_days} days, ${consistencyNote}.`;
  }
